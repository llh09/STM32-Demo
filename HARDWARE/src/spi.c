...
{
		/* 先发一个 */
	while(SPI_I2S_GetFlagStatus( SPI1, SPI_I2S_FLAG_TXE)==RESET);//	等待上次数据发送完成  
	SPI_I2S_ClearFlag(SPI1, SPI_I2S_FLAG_TXE);//读数据就请标志了（可不要）
	SPI_I2S_SendData(SPI1, data);//发送本次数据
	/*想要接收 */
	while(SPI_I2S_GetFlagStatus( SPI1, SPI_I2S_FLAG_RXNE)==RESET);//    等待接收到数据
	SPI_I2S_ClearFlag(SPI1, SPI_I2S_FLAG_RXNE);//读数据就请标志了（可不要）
	return SPI_I2S_ReceiveData(SPI1);//	读取并返回数据
}
